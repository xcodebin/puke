...
cc.Class({
    ...
    onLoad: function () {
       ...
       window.socket.emit('renwu',param);
       window.socket.on(this.user_name+'renwu',(msg)=>{
           console.log(msg);
           msg=JSON.parse(msg);
           if(this.flag===true){
               if(msg.status===false){
       ...
       window.socket.on(this.user_name+'addCoin',(msg)=>{
            msg=JSON.parse(msg);
           if(msg==="200"){
               this.flag=true;
               console.log(msg);
    ...
    GetRJ:function(){
        ...
        var mmm={userid:this.userid,name:this.user_name,num:0,type:'银币'};
         switch(this.renwu_t){
             case 5:  mmm.num=500;window.socket.emit('addCoin',mmm);this.prizeBox.active=true;this.prize.string='500银币';this.yinbi3.active=true;break;
             case 10: mmm.num=1000;window.socket.emit('addCoin',mmm);this.prizeBox.active=true;this.prize.string='1000银币';this.yinbi2.active=true;break;
             case 20: mmm.num=2000;window.socket.emit('addCoin',mmm);this.buttonhuoqu.active=false;this.prizeBox.active=true;this.prize.string='2000银币';this.yinbi1.active=true; break;
             default: console.log('errors');break;
         }
     },
    close:function(){
