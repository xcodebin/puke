...
cc.Class({
    ...
    onLoad: function () {
        ...
        window.socket.emit('System', param);
        window.socket.emit('Allfriend', param);

        // 系统消息 
        window.socket.on('System' + m.name, (msg) => {
            msg = JSON.parse(msg);
            console.log(msg);
        ...
        //加好友消息 
        window.socket.on('Gfriend' + m.name, (msg) => {
        ...
        window.socket.on(m.name + 'res', (msg) => {
            msg = JSON.parse(msg);
            //msg 为好友回应后的相应 
            console.log(msg);
        });
        //展示你的好友 
        window.socket.on('Allfriend' + m.name, (msg) => {
            msg = JSON.parse(msg);
            if (msg.status === true) {
                //msg.msg数组 为好友信息 
        ...
        window.socket.on('homeChat' + m.name, (msg) => {
            msg = JSON.parse(msg);
            if (msg.type === 0) {
                if (msg.statu === 200) {
                    // 没有聊过天 
                ...
                window.socket.emit('homeChat', msg);
            } else {
                console.log('baocun');
            }
        });
