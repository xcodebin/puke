...
cc.Class({
    ...
    properties: {
        ...
        slider: cc.Slider,
        Sliderprogressbar: cc.ProgressBar,
        room: '',
        userid: '',
        id: '',
        money: 0,
    ...
    onLoad: function () {
        ...
        var message;
        var xxx = { name: m.name };
        window.socket.emit('homeCoin', xxx);
        ...
        window.socket.on('home' + m.name, (msg) => {
            msg = JSON.parse(msg);
        ...
        //window.socket=window.io('http://192.168.0.100:7777');
        //  window.socket.emit('qingkong',{a:0});
        console.log(self.type + 'type ' + JSON.parse(cc.sys.localStorage.getItem("roomid")).coin);
        window.socket.emit('UpdateRoom', this.room);
        window.socket.on('UpdateRoomBack' + this.room, (msg) => {
            ...
            if (m.name == a.number1) {
                ...
                window.socket.emit('coming1', mmm);
            }
            if (m.name == a.number2) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming2', mmm);
            }
            if (m.name == a.number3) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming3', mmm);
            }
            if (m.name == a.number4) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming4', mmm);
            }
            if (m.name == a.number5) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming5', mmm);
            }
            if (m.name == a.number6) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming6', mmm);
            }
            if (m.name == a.number7) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming7', mmm);
            }
            if (m.name == a.number8) {
                var mmm = { roomid: self.room, name: m.name, money: self.allmoney, change: 0, action: '', coinType: self.type };
                window.socket.emit('coming8', mmm);
            }
        });
        window.socket.on(self.room + 'come1', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come2', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come3', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come4', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come5', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come6', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come7', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on(self.room + 'come8', (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on('tuifang' + self.room, (data) => {
        ...
        window.socket.on(self.room + 'xiazhuChanged', (msg) => {
            msg = JSON.parse(msg);
            if (self.maxBet < msg.num) {
                self.maxBet = msg.num;
                self.money = self.maxBet;
                self.jiazhu = 2 * self.money;
            }
        ...
        window.socket.on(self.room + 'moneyChanged', (msg) => {
            msg = JSON.parse(msg);
            // self.item.string+=msg.name+':'+msg.action+msg.change+'\n';
            if (msg.action == 'check') {
        ...
        window.socket.on('zhuang' + self.room, (msg) => {
            msg = JSON.parse(msg);
            if (msg.name == self.player1Name.string) {
        ...
        window.socket.on('selectname' + self.room, (msg) => {
            msg = JSON.parse(msg);
            if (msg.name == self.player1Name.string) {
        ...
        window.socket.on('huojiang2' + self.room, (msg) => {
            msg = JSON.parse(msg);
            if (msg.name == self.player1Name.string) {
        ...
        window.socket.on('paipaipai' + self.room, (msg) => {
            ...
            console.log(msg.name + "carddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd");
            console.log(msg.name + "carddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd");
            if (msg.name != m.name) {
                if (msg.name == self.player1Name.string) {
                    var card1;
        ...
        window.socket.on(self.room + 'dichi', (msg) => {
            msg = JSON.parse(msg);
            if (msg.num > 10000) {
        ...
        //           window.socket.emit('xintiao',{message:'我现在在线吗?',userid:self.userid});
        ...
        //             }, 5);
        //         }, 7);
        //         window.socket.on('xintiaohuidiao'+self.userid,(a)=>{
        //             a=JSON.parse(a);
        //               if(a.message=='你还在线'){
        //                   self.xintiao=1;
        //               }
        ...
        self.scheduleOnce(function () {
            window.socket.emit('zhunbei', { roomid: self.room, userid: self.userid, name: m.name });
        }, 2);
        //  self.usermoney.string=self.allmoney;

        self.xiazhu.string = self.xiazhumoney;
        //监听用户顺序
        window.socket.on(self.room, function (data) {//一直监听  操作需要执行动作的玩家
            data = JSON.parse(data);
            console.log("打印打印前" + data.money);
            self.money = data.money;
            self.jiazhu = 2 * data.money;
            ...
            if (self.allmoney > self.jiazhu) {
                self.jiazhunum.string = '加' + 2 * self.money;
            } else {
            ...
            if (data.id == self.userid) {
                window.socket.emit('selectname', { userid: self.userid, roomid: self.room });
                self.xianshi();
                console.log("显示了");
        ...
        window.socket.on(self.room + 'jieshu', function (data) {
            data = JSON.parse(data);
            ...
            if (self.userid == data.id && data.statusa == 66) {
                ...
                console.log('在得奖这里获得了      allmoney ' + self.allmoney + 'all1' + self.all1 + 'type' + self.type);
                var mmsg = { roomid: self.room, name: m.name, num: self.allmoney, action: '' };
                window.socket.emit('changeMoney', mmsg);
                window.socket.emit('baocunmoney', { name: m.name, money: self.allmoney, type: self.type });
                //   var xxx={name:m.name};
                window.socket.emit('homeCoin', xxx);
                window.socket.emit('huojiang1', { name: m.name, room: self.room, num: data.num });

                if (self.flagJ == 0) {
                    window.socket.emit('exp', { id: self.userid });
                    self.flagJ == 1;
                }
                self.yincang();

                //     window.socket.on('dengji',(msg)=>{
                ...
                window.socket.emit('homeCoin', xxx);
                //  window.socket.emit('exp',{id:self.userid,ss:'lose'});//一开始游戏,先加一部分经验  
                //         window.socket.on('dengji',(msg)=>{
                // self.resultWindow.active=true;
        ...
        window.socket.on(self.room + 'only', function (data) {//只剩一个人的时候通过此监听
            data = JSON.parse(data);
            if (self.userid == data.id) {
                window.socket.emit('xiazhu', { id: self.userid, roomid: self.room, money: self.xiazhumoney });
                console.log('就剩我一个人');
            }

        });
        window.socket.on('qipaile' + self.room, (msg) => {
            msg = JSON.parse(msg);
        ...
        window.socket.on('onefapai' + self.room, (data) => {

            data = JSON.parse(data);
            self.onefapai = 1;
        });
        //开始游戏的时候  给用户分配顺序id
        window.socket.on(self.userid + 'one', function (data) {
            self.maxBet = 0;
            window.socket.emit('homeCoin', xxx);
            self.onefapai = 11;
            data = JSON.parse(data);
            self.flagQ = 0;
            console.log('datawuxian+++++++++++++++++++++++++++++' + data);
            ...
            if (data.name == "zhuang") {
                console.log("你是庄家");
                window.socket.emit('zhuang', { roomid: self.room, name: m.name });
            } else if (data.name == "xiaomang") {
                console.log("你是小盲注");
                self.flagJ == 0;
                self.audioMng.playBet();
                //下注
                window.socket.emit('xiaomang', { id: self.userid, roomid: self.room, money: fangfei * 0.01 });
                ...
                window.socket.emit('changexiazhu', xmsg);
                window.socket.emit('findDichi', { roomid: self.room });
            }
                ...
                self.allmoney -= fangfei * 0.02;
                window.socket.emit('damangzhu', { id: self.userid, roomid: self.room, money: fangfei * 0.02 });
                ...
                window.socket.emit('findDichi', { roomid: self.room });
                //下注
                window.socket.emit('xiazhu', { id: self.userid, roomid: self.room, money: fangfei * 0.02 });//测试此处，向服务器发送请求，要求发牌和轮流说话
            ...
            if (self.userid == data.id) {
                window.socket.emit('xiazhu', { id: self.userid, roomid: self.room, money: self.all1 });
        ...
        window.socket.on('lunci', function (data) {
            data = JSON.parse(data);
            if (data.num == 1) {
                //执行发第一轮底牌函数
                console.log("第一轮发牌");
                ...
                //     self.mmsg = { roomid: self.room, name: m.name, num: 0, action: ' ' };

                // window.socket.emit('changeMoney', self.mmsg);
                console.log(self.allmoney + "jieshu -----====");
    ...
    callback: function (event) {
        ...
        if (self.jiazhu < self.allmoney) {
            self.jiazhunum.string = 'Raise' + self.jiazhu;
        } else {
    ...
    buttonqipai: function () {
        ...
        var m = JSON.parse(cc.sys.localStorage.getItem("UserMessage"));
        var self = this;
        window.socket.emit('qipai', { id: self.userid, name: m.name, roomid: self.room, money: self.money, onepai: self.onefapai });
        self.yincang();
        self.flagQ = 1;
        window.socket.emit("baocunpai", { uid: self.userid, pai: self.shoupai });
    ...
    buttonjiazhu: function () {
        ...
        if (self.jiazhu < self.allmoney) {
            ...
            window.socket.emit('jiazhu', { id: self.userid, roomid: self.room, money: self.jiazhu, onepai: self.onefapai });
            console.log("jiazhu" + self.id);
            self.xmsg = { roomid: self.room, name: m.name, num: self.xiazhumoney };
            window.socket.emit('changexiazhu', self.xmsg);
            window.socket.emit('findDichi', { roomid: self.room });
        } else {
            ...
            window.socket.emit('changexiazhu', self.xmsg);
            window.socket.emit('findDichi', { roomid: self.room });
            window.socket.emit('allin', { id: self.userid, roomid: self.room, money: (self.all1), onepai: self.onefapai });
    ...
    buttongenzhu: function () {
        ...
        if (self.allmoney + self.xiazhumoney <= self.money) {
            ...
            if (self.onefapai == 11 && self.usertype == 'damang') {
                window.socket.emit('onenumfapai', { flag: self.onefapai, type: self.usertype, roomid: self.room });
            ...
            if (self.onefapai == 11 && self.usertype == 'damang') {
                ...
                window.socket.emit('xiazhu', { id: self.userid, roomid: self.room, money: self.money, onepai: self.onefapai });
                console.log('操作了 ' + self.userid);
            }
        }
    ...
    tuifang: function () {
        ...
        window.socket.emit('tuifang', param);
        window.socket.emit('tuifangqipai', { id: self.userid, roomid: self.room });
