...
cc.Class({
	...
	onLoad: function () {
		...
		window.socket.on(m.name + 'addfriend', (msg) => {
			msg = JSON.parse(msg);
		...
		window.socket.on(m.name + 'req', (msg) => {
			msg = JSON.parse(msg);
			var self = this;
			if (msg.status === true) {
				console.log(msg.msg);
				var mmm = {
					M_name: msg.reqname, Y_name: msg.resname, myuserid: msg.reqid, youruserid: msg.resid, id: msg.id
				};
				cc.sys.localStorage.setItem("addfriend", JSON.stringify(mmm));
				var param = { userid: msg.resid, name: res.resname };
				// window.socket.emit('Gfriend',param);
		...
		window.socket.on(m.name + 'chat', (msg) => {
			var self = this;
			msg = JSON.parse(msg);
			if (msg.status == true) {
	...
	tongyi: function () {
		...
		window.socket.emit('addfriend', param);
	},
	jujue: function () {
		this.msgwindow.active = false;
		var msg = JSON.parse(cc.sys.localStorage.getItem("YN"));
	...
	send: function () {
		...
		if (Msg === null || Msg === undefined || Msg === "") {
			...
			window.socket.emit('Roomchat', param);
			this.editBoxText.string = '';
			this.type = 0;
		}
